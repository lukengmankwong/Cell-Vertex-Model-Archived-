...
void Cell::calcG()
{
	...
	director = 0.01*Vec(1, (lambda-G[0])/G[1])/((lambda-G[0])/G[1]);
}
...
void Cell::calcA()
{
	...
	for (int i = 0; i < vertex_keys.size(); i++) {
		A += vertex_map.at(vertex_keys[i]).getR().x()*vertex_map.at(vertex_keys[(i+1)%vertex_keys.size()]).getR().y()
			- vertex_map.at(vertex_keys[(i+1)%vertex_keys.size()]).getR().x()*vertex_map.at(vertex_keys[i]).getR().y();
	} A *= 0.5;
	dA = A - A_old;
}
