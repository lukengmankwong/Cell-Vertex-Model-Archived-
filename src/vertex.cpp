...
Vertex::Vertex(Global* g, int id, Point r) : g(g), id(id), r(r), force(Vec(0,0)) { }
...
Vec Vertex::calcSurfaceForce()
{
	...
	for (int c : cell_contacts) 
	{
		...
		double dAdx = S*0.5*(g->vertexMap().at(c_vertex_keys[(j+1)%n]).getR().y() - g->vertexMap().at(c_vertex_keys[(j-1+n)%n]).getR().y());
		double dAdy = S*0.5*(g->vertexMap().at(c_vertex_keys[(j-1+n)%n]).getR().x() - g->vertexMap().at(c_vertex_keys[(j+1)%n]).getR().x());
...
Vec Vertex::calcLineForce()
{
	...
	for (int e : edge_contacts)
	{
		...
		double x_diff = r.x() - g->vertexMap().at(v_).getR().x();
		double y_diff = r.y() - g->vertexMap().at(v_).getR().y();
