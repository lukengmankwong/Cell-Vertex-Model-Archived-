...
bool doughnut(const Point& p) { return p.x()*p.x() + p.y()*p.y() > 0.075 && p.x()*p.x() + p.y()*p.y() < 0.25; }
bool square(const Point& p) { return std::fabs(p.x()) < 100 && std::fabs(p.y()) < 100; }
